...
// writeIntcodes prints an intcode program to standard output.
func writeIntcodes(prog []int) {
	var results []string
	for _, v := range prog {
		results = append(results, strconv.Itoa(v))
	}

	// Write processed program intcodes to standard output.
	w := csv.NewWriter(os.Stdout)
	if err := w.Write(results); err != nil {
		log.Fatalln("Error writing program to output:", err)
	}
	w.Flush()
}
